...
def spawn(
    ...
    log: Optional[LogFn] = None,
    out=None,
    tty: bool = False,   # accepted for compatibility; airodump captures fine on a pipe
    ...
    Output goes to a caller-provided file handle (for diagnostics) or /dev/null,
    never an undrained PIPE (airodump is chatty and a full pipe buffer would
    freeze it). Parsing is done off the CSV files, never this stream.
    """
    argv = [str(a) for a in argv]
    if log:
        log("$ " + " ".join(argv) + "  &")
